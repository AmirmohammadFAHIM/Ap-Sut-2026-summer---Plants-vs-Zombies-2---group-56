...
public class RegexHelper {
    ...
    public static final String LOGIN_COMMAND_PATTERN =
            "^\\s*login\\s+-u\\s+(?<username>\\S+)\\s+-p\\s+(?<password>\\S+)(?:\\s+(?<stayLoggedIn>-stay-logged-in))?\\s*$";
    ...
    public static final String PROFILE_CHANGE_USERNAME = "(?i)^menu\\s+profile\\s+change-username\\s+-u\\s+(?<username>\\S+)$";
    public static final String PROFILE_CHANGE_NICKNAME = "(?i)^menu\\s+profile\\s+change-nickname\\s+-(n|u)\\s+(?<nickname>.+)$";
    ...
    public static final String COLLECTION_SHOW_PLANT = "(?i)^menu\\s+collection\\s+show-plant\\s+-p\\s+(?<name>[a-zA-Z_]+)$";
    public static final String COLLECTION_SHOW_ZOMBIE = "(?i)^menu\\s+collection\\s+show-zombie\\s+-z\\s+(?<name>[a-zA-Z_]+)$";
    public static final String COLLECTION_UPGRADE_PLANT = "(?i)^menu\\s+collection\\s+upgrade-plant\\s+-p\\s+(?<name>[a-zA-Z_]+)$";
    public static final String COLLECTION_BUY_PLANT = "(?i)^menu\\s+collection\\s+purchase-plant\\s+-p\\s+(?<name>[a-zA-Z_]+)$";

    public static final String SETTINGS_CHANGE_DIFFICULTY = "(?i)^menu\\s+settings\\s+change-difficulty\\s+-l\\s+(?<level>\\d+)$";
    ...
    public static final String SHOP_BUY = "(?i)^shop\\s+buy\\s+-i\\s+(?<itemId>\\d+)\\s+-n\\s+(?<count>\\d+)(?:\\s+-t\\s+(?<plantType>[a-zA-Z_]+))?$";
    ...
    public static final String WALLET_CHEAT = "(?i)^cheat\\s+add\\s+(?<amount>\\d+)\\s+(?<type>coin|coins|diamond|diamonds)$";
    ...
    public static final String GAME_PLANT_PLANT = "(?i)^plant\\s+plant\\s+-t\\s+(?<type>[a-zA-Z_]+)\\s+-l\\s+\\((?<x>\\d+),\\s*(?<y>\\d+)\\)$";
    ...
    public static final String GAME_SHOW_TILE_STATUS = "(?i)^show\\s+tile\\s+status\\s+-l\\s+\\((?<x>\\d+),\\s*(?<y>\\d+)\\)$";
    public static final String GAME_ZOMBIES_INFO = "(?i)^zombies\\s+info$";
    public static final String GAME_CHEAT_SPAWN_ZOMBIE = "(?i)^cheat\\s+spawn-zombie\\s+-t\\s+(?<type>[a-zA-Z_]+)\\s+-l\\s+\\((?<x>\\d+),\\s*(?<y>\\d+)\\)$";

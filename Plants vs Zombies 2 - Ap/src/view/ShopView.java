...
public class ShopView extends View {
    ...
    public void input() {
        ...
        if (listMatcher.matches()) {
            ...
            switch (itemId) {
                case 1: shopMenu.purchase("pot", count); break;
                case 2: shopMenu.purchase("plantfood", count); break;
                case 3:
                    for(int i=0; i<count; i++) shopMenu.randomPurchase();
                    break;
                case 4:
                    if(plantType == null) System.out.println("Error: -t <plant_type> is required for specific seeds.");
                    else {
                        for(int i=0; i<count; i++) shopMenu.normalPurchase(plantType.toUpperCase());
                    }
                    break;
                case 5: shopMenu.purchase("exchange", count); break;
